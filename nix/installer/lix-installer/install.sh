...
main() {
  ...
  *windows*)
  ...
  # echo "# using _url: $_url target _file: $_file" && return
...
downloader() {
  ...
  if [ "$1" = --check ]; then
    ...
    if [ -n "$_ciphersuites" ]; then
      ...
      echo "curl $_retry --proto '=https' --tlsv1.2 --ciphers "$_ciphersuites" --silent --show-error --fail --location "$1" --output "$2""
      # echo "debug curl" && exit 2
      _status=$?
    else
